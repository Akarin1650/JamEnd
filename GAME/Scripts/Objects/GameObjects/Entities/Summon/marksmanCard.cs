...
{
    public class marksmanCard : TextureButton
    {
        ...
        private List<string> names = new List<string>() { "Dolores", "Rowena", "Richard", "Seraphin", "Roger" };


        public override void _Ready()
        {
            SetArcher = new SetClass(myArcher, 1);
            myEfficiency = myArcher.Efficiency;
            mystrength = myArcher.Strength;
            myMagic = myArcher.Magic;
            myCharisma = myArcher.Charisma;
            myArmor = myArcher.Armor;
            EntityManager.GetInstance().summonText.Clear();
            EntityManager.GetInstance().SetSummonAttribute(SetArcher, myArcher);
            ...
            if (isDragged)
            {
                ...
                if (Input.IsActionJustReleased("RMB"))
                {
                    ...
                    if (Teleport.GetInstance().isCardIn && !GameManager.GetInstance().isDiscarding)
                    {
                        ...
                        SoundManager.GetInstance().PlaySfxSound(SfxName.SUMMON_FOX, 0, -5);
...
TrashArea.GetInstance().anim.PlayBackwards("Spawn");
        ...
        private void OnPressed()
        {
            ...
            myMagic, myCharisma, myArmor, myArcher.Element, this, myArcher, attributeOne);
        ...
        public override void _Input(InputEvent pEvent)
        {
            ...
            {
                isDragged = true;
                UtilsSignals.GetInstance().EmitSignal(nameof(UtilsSignals.OnCardSelected), myArcher);
            }
        }
    }
}
